...
void
CGame::ModifyCurrentTrialNumber( )
{
    UpdateRestOfItems( );
         ...
         == static_cast< unsigned short >( TRIAL_NUMBER::SECOND ) )
    {
        ...
        if ( GetCurrentFrame( )->GetSecondTrialItems( ) + GetCurrentFrame( )->GetFirstTrialItems( )
                 == puUtil->GetMaxItems( )
        ...
        else
        {
            ...
            unsigned uCurrentTotal = GetCurrentFrame( )->GetSecondTrialItems( )
                                     + GetCurrentFrame( )->GetFirstTrialItems( );
        ...
        if ( GetCurrentFrame( )->Get10thFrame( ) )
        {
            if ( GetCurrentFrame( )->GetFirstTrialItems( ) == puUtil->GetMaxItems( )
                 || GetCurrentFrame( )->GetFirstTrialItems( )
                            + GetCurrentFrame( )->GetSecondTrialItems( )
                        == puUtil->GetMaxItems( ) )
            {
                ...
                if ( GetCurrentFrame( )->GetFlags( ) == FLAGS::STRIKE )
                {
                    ...
                    if ( FrPrevious->GetFlags( ) == FLAGS::STRIKE )
                    {
                        puUtil->Log( "2nd trial TOTAL BEFORE 10th XXX: "
                                     + std::to_string( uTotal ) );
                        uTotal += puUtil->GetMaxItems( );
                        puUtil->Log( "2nd trial TOTAL AFTER 10th XXX: "
                                     + std::to_string( uTotal ) );

                        // Special case where all previous tries were X, that's why it is hardcoded
                        if ( uTotal == 280 )
                        {
                            puUtil->Log( "2nd trial TOTAL BEFORE 10th All previous X case: "
                                         + std::to_string( uTotal ) );
                            uTotal += puUtil->GetMaxItems( );
                            puUtil->Log( "2nd trial TOTAL AFTER 10th All previous X case: "
                                         + std::to_string( uTotal ) );
                        }
                    }
                }
    ...
    else
    {
        if ( GetCurrentFrame( )->Get10thFrame( ) )
        {
            if ( GetCurrentFrame( )->GetFirstTrialItems( ) == puUtil->GetMaxItems( ) )
            {
                ...
                {
                    uRestOfItems
                        = puUtil->GetMaxItems( ) - GetCurrentFrame( )->GetSecondTrialItems( );
                }
            }
            else
            {
