...
void CLane::ModifyPlayer(unsigned uIndex, const std::string& sName)
{
	std::shared_ptr<CPlayer> pPlayer(new CPlayer(sName));
	vpPlayer.at(uIndex) = std::move(pPlayer);
}

void CLane::AddPlayer(const std::string& sName)
{
	std::shared_ptr<CPlayer> pPlayer(new CPlayer(sName));
...
void CLane::Play()
{
	...
	while (!bFinish)
	{
		for (auto& pPlayer : vpPlayer)
		{
		    ...
		    {
		    	puUtil->Log(pPlayer->GetName(), "Current Frame number: " + std::to_string(pPlayer->GetGame()->GetCurrentFrameNumber()));
		    	if (pPlayer->GetGame()->GetCurrentFrame()->GetCurrentTrialNumber() == static_cast<unsigned short>(TRIAL_NUMBER::FIRST))
		    	{
		    	...
		    	{
		    		if (pPlayer->GetGame()->GetCurrentFrame()->GetCurrentTrialNumber() == static_cast<unsigned short>(TRIAL_NUMBER::SECOND))
		    		{
			    		//Log("2nd trial rest of items: " + std::to_string(pPlayer->GetGame()->GetRestOfItems()));
		    	...
		    	else
		    	{
		    		...
		    		else if (pPlayer->GetGame()->GetCurrentFrame()->GetCurrentTrialNumber() == static_cast<unsigned short>(TRIAL_NUMBER::SECOND))
		    		{
			    		//Log("2nd trial rest of items: " + std::to_string(pPlayer->GetGame()->GetRestOfItems()));
		    	...
		    	if (pPlayer->GetGame()->GetCurrentFrame()->GetCurrentTrialNumber() == static_cast<unsigned short>(TRIAL_NUMBER::THIRD))
		    	{
		    		if (pPlayer->GetGame()->GetCurrentFrame()->GetCurrentTrialNumber() != static_cast<unsigned short>(TRIAL_NUMBER::SECOND))
			    	{
			    		if (pPlayer->GetGame()->GetCurrentFrame()->Get10thFrame() && 
			    			pPlayer->GetGame()->GetCurrentFrame()->GetFirstTrialItems() == puUtil->GetMaxItems())
				    	{
				    		WaitForKick(pPlayer->GetGame()->GetRestOfItems());
				    	}
				    	else
		    	...
		    	puUtil->Log(pPlayer->GetName(), "Current frame flag: " + pPlayer->GetGame()->GetCurrentFrame()->GetFlagsToString() + "\n\n");
		    }
		}
		...
		for (auto& pPlayer : vpPlayer)
		{
			if (pPlayer->GetGame()->GetCurrentFrame()->Get10thFrame())
			{
				puUtil->Log(pPlayer->GetName(), "Total of the game: " + std::to_string(pPlayer->GetGame()->GetTotal()));
				bFinish = true;
			}
