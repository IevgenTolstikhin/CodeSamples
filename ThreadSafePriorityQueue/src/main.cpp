...
usage( )
{
    std::cout << "thread_safe_priority_queue [--number_of_clients <number_of_clients>], "
              << std::endl
              << "where <number_of_clients> need to be between " << min_number_of_clients << " and "
              << max_number_of_clients << std::endl;
}
...
main( int argc, char** argv )
{
    ...
    if ( argc == 1 )
    {
        do
        {
            std::cout << "Please input number of clients from " << min_number_of_clients << " to "
    ...
    std::string logFile( std::string( "ClientServer.log" ) );
...
#endif
            std::cerr << std::endl;
        } while ( total_finish != 'y' && total_finish != 'Y' && total_finish != 'n'
                  && total_finish != 'N' );
